...
def getQuizes(user):
    ...
    if user == 'admin' or user == 'fabioja':
        cursor.execute("SELECT id, \
            numb from QUIZ".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    else:
        cursor.execute("SELECT id, numb from QUIZ where \
            release < '{0}'".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
...
def getUserQuiz(userid, quizid):
    conn = sqlite3.connect(DBNAME)
    cursor = conn.cursor()
    cursor.execute("SELECT sent, \
        answer,result from USERQUIZ where userid = '{0}' and quizid = {1} order by sent desc".format(userid, quizid))
    info = [reg for reg in cursor.fetchall()]
    conn.close()
    return info
...
def setUserQuiz(userid, quizid, sent, answer, result):
    ...
    cursor.execute("insert into \
        USERQUIZ(userid,quizid,sent,answer,result) values (?,?,?,?,?);", (userid, quizid, sent, answer, result))
    conn.commit()
    conn.close()
